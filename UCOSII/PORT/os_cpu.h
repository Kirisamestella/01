...
void       OSCtxSw(void);           // 一个用汇编编写的0x80的中断向量入口函数,
                                    // 函数把被中止任务的断点指针和CPU的寄存器值保存到该任务的堆栈中，
                                    // 并从待运行任务的堆栈中得到其堆栈指针，把待运行任务堆栈中的存储
                                    // 的CPU通用寄存器的内容恢复到CPU的通用寄存器中，最后使CPU获得待运行任务的断点指针。
void       OSIntCtxSw(void);        // 中断切换函数
void       OSStartHighRdy(void);    // 高优先级就绪任务启动函数 
