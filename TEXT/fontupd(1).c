...
//fsize:整个文件大小
//pos:当前文件指针位置
u32 fupd_prog(u16 x,u16 y,u8 size,u32 fsize,u32 pos,u16 point_color,u16 back_color)
{
	float prog;
	u8 t=0XFF;
	...
	if(t!=prog)
	{
		LCD_ShowString(x+3*size/2,y,240,320,size,"%",point_color,back_color,0);		
		t=prog;
		if(t>100)t=100;
		LCD_ShowNum(x,y,t,3,size,point_color,back_color,0);//显示数值
	}
	return 0;					    
} 
//更新某一个
...
//返回值:0,成功;其他,失败.
u8 updata_fontx(u16 x,u16 y,u8 size,u8 *fxpath,u8 fx,u16 point_color,u16 back_color)
{
	u32 flashaddr=0;								    
	FIL * fftemp;
	u8 *tempbuf;
 	...
 	if(rval==0)	 
	{
		...
		while(res==FR_OK)//死循环执行
		{
			...
			SPI_Flash_Write(tempbuf,offx+flashaddr,4096);		//从0开始写入4096个数据  
	  		offx+=bread;	  
			fupd_prog(x,y,size,fftemp->fsize,offx,point_color,back_color);	 			//进度显示
...
//返回值:0,更新成功;
//		 其他,错误代码.	  
u8 update_font(u16 x,u16 y,u8 size,u16 point_color,u16 back_color)
{	
	...
	ftinfo.fontok=0XFF;
 	...
 	LCD_ShowString(x,y,240,320,size,"Updating UNIGBK.BIN",point_color,back_color,0);		
	res=updata_fontx(x+20*size/2,y,size,unigbk_path,0,point_color,back_color);			//更新UNIGBK.BIN
	if(res)return 1;
 	LCD_ShowString(x,y,240,320,size,"Updating GBK12.BIN  ",point_color,back_color,0);
	res=updata_fontx(x+20*size/2,y,size,gbk12_path,1,point_color,back_color);			//更新GBK12.FON
	if(res)return 2;
	LCD_ShowString(x,y,240,320,size,"Updating GBK16.BIN  ",point_color,back_color,0);
	res=updata_fontx(x+20*size/2,y,size,gbk16_path,2,point_color,back_color);			//更新GBK16.FON
	if(res)return 3;
	LCD_ShowString(x,y,240,320,size,"Updating GBK24.BIN  ",point_color,back_color,0);
	res=updata_fontx(x+20*size/2,y,size,gbk24_path,3,point_color,back_color);			//更新GBK24.FON
	if(res)return 4;	   
	//全部更新好了
	ftinfo.fontok=0XAA;
