...
u8 wireless_play(void)
{
 	...
 	if(res==0)//确认键按下了,同时按键正常创建
	{
		...
		{
			if(res==0)window_msg_box((lcddev.width-200)/2,(lcddev.height-70)/2-15,200,70,(u8*)wireless_remind_msg_tbl[gui_phy.language],(u8*)APP_REMIND_CAPTION_TBL[gui_phy.language],12,0,0);
			delay_ms(500);
			res++;
			if(res>5)return 0;//超时未检测到,退出
		}				  
   		gui_fill_rectangle(0,20,lcddev.width,lcddev.height-20,LGRAY);	//填充背景色
		...
		caption=(u8*)wireless_mode_tbl[gui_phy.language][mode];	//标题
 		...
 		cbtn=btn_creat(0,lcddev.height-20,40,19,0,0x03);//创建文字按钮
		...
		{	 
			...
			{	    
				...
				{	
				 	if(tp_dev.y[0]<(lcddev.height-20)&&tp_dev.y[0]>(20+1))	//在画图区域内
					{		
						x=tp_dev.x[0];
						y=tp_dev.y[0];
			...
			if(tmp_buf[4]&0X7F)	//需要处理
			{
				switch(tmp_buf[4]&0X7F)
				{
					case 1://正常画点
						...
						break;
					case 2://清除
						gui_fill_rectangle(0,20,lcddev.width,lcddev.height-40,LGRAY);//清除屏幕		
						break;
					case 3://退出						  
						SPI1_SetSpeed(SPI_SPEED_2);//设置回高速.18Mhz 
						rval=1;//标志退出
						break; 
				} 
			}else 
