...
{		
  	if(mode&0X01)app_gui_tcbar(0,0,lcddev.width,APP_FB_TOPBAR_HEIGHT,0x02);									//下分界线
	if(mode&0X02)app_gui_tcbar(0,lcddev.height-APP_FB_BTMBAR_HEIGHT,lcddev.width,APP_FB_BTMBAR_HEIGHT,0x01);//上分界线
	if(mode&0X04)gui_show_strmid(0,0,lcddev.width,APP_FB_TOPBAR_HEIGHT,WHITE,16,topname);	  
} 
//在指定位置显示一个大字符
...
u8 app_listbox_select(u8 *sel,u8 *top,u8 * caption,u8 *items[],u8 itemsize)
{
	...
	if(*sel>=itemsize||*top>=itemsize)return 2;//参数错误/参数非法
 	app_filebrower(caption,0X07);				//显示标题  
   	tlistbox=listbox_creat(0,APP_TEST_TOPBAR_HEIGHT,lcddev.width,lcddev.height-APP_TEST_TOPBAR_HEIGHT*2,1,16);	//创建一个filelistbox
	...
	if(rval==0)//成功添加了条目
	{
		...
		rbtn=btn_creat(lcddev.width-41,lcddev.height-APP_FB_BTMBAR_HEIGHT,40,19,0,0x03);//创建返回文字按钮
		okbtn=btn_creat(0,lcddev.height-APP_FB_BTMBAR_HEIGHT,40,19,0,0x03);				//创建确认文字按钮
...
{ 
	...
	{	
		//主界面图标检测,9个. 
 		for(i=0;i<9;i++)
		{
	 		res=f_open(f_check,(const TCHAR*)mui_icos_path_tbl[i],FA_READ);//只读方式打开文件
			if(res)break;		//打开失败   
...
const u8* APP_SYSTEM_SYSICOPATH="0:/SYSTEM/SYSICO";	//SYSICO文件夹路径
