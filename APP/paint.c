...
u8 paint_play(void)
{
	...
	{	   
		...
		{
			app_gui_tcbar(0,lcddev.height-APP_TEST_TOPBAR_HEIGHT,lcddev.width,APP_TEST_TOPBAR_HEIGHT,0x01);	//上分界线
		...
		pset_bkctbl=gui_memex_malloc(150*96*2);//申请内存
		...
		{		 
			...
			if(key>=2)						//KEY1/WK_UP 按下,颜色设置
			{
				...
				if(key==2)paint_pen_color_set((lcddev.width-150)/2,(lcddev.height-96)/2,&pencolor,(u8*)pen_set_tbl[gui_phy.language][0]);	//KEY1按下
	...
	{	   
		app_read_bkcolor((lcddev.width-150)/2,(lcddev.height-80)/2,150,60,pset_bkctbl);		//读取背景色
		res=window_msg_box((lcddev.width-150)/2,(lcddev.height-80)/2,150,60,"",(u8*)APP_SAVE_CAPTION_TBL[gui_phy.language],12,0,0X03);
		app_recover_bkcolor((lcddev.width-150)/2,(lcddev.height-80)/2,150,60,pset_bkctbl);	//恢复背景色
		if(res==1)//需要保存
		{
			gui_memex_free(pset_bkctbl);	//释放内存,以供bmp编码申请.
			if(bmp_encode(pname,0,0,lcddev.width,lcddev.height,1))//编码BMP文件
			{
				window_msg_box((lcddev.width-150)/2,(lcddev.height-80)/2,150,60,(u8*)APP_CREAT_ERR_MSG_TBL[gui_phy.language],(u8*)APP_REMIND_CAPTION_TBL[gui_phy.language],12,0,0);//提示SD卡是否存在
				delay_ms(2000);//等待2秒钟
			}
		}
	} 
