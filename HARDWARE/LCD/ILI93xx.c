...
{ 
	...
	GPIOC->ODR|=0X03C0; 	 //原0X07C0，改0X03C0，让LCD_LED先关闭，等初始化完毕再打开。
	//PORTB 推挽输出 	
...
void LCD_ShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode,u16 point_color,u16 back_color,u16 ms)
{  							  
    u8 temp,t1,t;
	u16 y0=y;
...
//color:颜色 
//num:数值(0~4294967295);	 
void LCD_ShowNum(u16 x,u16 y,u32 num,u8 len,u8 size,u16 point_color,u16 back_color,u16 ms)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,' ',size,0,point_color,back_color,ms);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,temp+'0',size,0,point_color,back_color,ms); 
	}
} 
//显示数字,高位为0,还是显示
//x,y:起点坐标
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowxNum(u16 x,u16 y,u32 num,u8 len,u8 size,u8 mode,u16 point_color,u16 back_color,u16 ms)
{  
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode&0X80)LCD_ShowChar(x+(size/2)*t,y,'0',size,mode&0X01,point_color,back_color,ms);  
				else LCD_ShowChar(x+(size/2)*t,y,' ',size,mode&0X01,point_color,back_color,ms);  
 				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,temp+'0',size,mode&0X01,point_color,back_color,ms); 
	}
} 
//显示字符串
//x,y:起点坐标
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_ShowString(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p,u16 point_color,u16 back_color,u16 ms)
{         
	u8 x0=x;
	width+=x;
	height+=y;
    ...
    {       
        ...
        LCD_ShowChar(x,y,*p,size,0,point_color,back_color,ms);
        x+=size/2;
        p++;
    }  
}
...
u8 table[table_x][table_y];
//画线
//x1,y1:起点坐标
//x2,y2:终点坐标  
void table_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2,u16 color)
{
	u16 t; 
	int xerr=0,yerr=0,delta_x,delta_y,distance; 
	int incx,incy,uRow,uCol; 
	...
	uRow=x1; 
	uCol=y1; 
    if(uRow>=table_x)uRow=table_x-1;
    if(uCol>=table_y)uCol=table_y-1;
	if(delta_x>0)incx=1; //设置单步方向 
	else if(delta_x==0)incx=0;//垂直线 
	else {incx=-1;delta_x=-delta_x;} 
	if(delta_y>0)incy=1; 
