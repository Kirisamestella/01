...
//此定义在main.c函数里面,如果不需要返回,直接去掉.
extern volatile u8 system_task_return;		//任务强制返回标志.


#define MSG_BOX_BTN1_WIDTH 			50
#define MSG_BOX_BTN2_WIDTH 			80
#define MSG_BOX_BTN_HEIGHT 			24
#define MSG_BOX_CAP_HEIGHT 			24
//在制定位置显示一个msg box
...
u8 window_msg_box(u16 x,u16 y,u16 width,u16 height,u8 *str,u8 *caption,u8 font,u16 color,u8 mode)
{
	...
	u16 temp,strheight=0;
 	...
 	if(width<130||height<(MSG_BOX_CAP_HEIGHT+font+5))return 0;	//尺寸错误
  	twin=window_creat(x,y,width,height,0,1|((7<<5)&mode),16);	//创建窗口 
	if(twin==NULL)return 0;
	twin->captionheight=MSG_BOX_CAP_HEIGHT;		//修改caption高度为MSG_BOX_CAP_HEIGHT
	...
	{
 		okbtn=btn_creat(x+offx,y+height-MSG_BOX_BTN_HEIGHT-6,offy,MSG_BOX_BTN_HEIGHT,0,0x02);//创建OK按钮
	...
	{
		if(mode&(1<<0))cancelbtn=btn_creat(x+offx*2+MSG_BOX_BTN1_WIDTH,y+height-MSG_BOX_BTN_HEIGHT-6,offy,MSG_BOX_BTN_HEIGHT,0,0x02);//创建cancel按钮
	...
	else
	{
		...
		{
			rval=1;//不需要进入while	
			strheight=height-(MSG_BOX_CAP_HEIGHT+5);
		}else strheight=height-(MSG_BOX_CAP_HEIGHT+50+5); 
		...
		gui_show_string(str,x+offx,y+MSG_BOX_CAP_HEIGHT+offy,width-offx*2,strheight,font,color);	//显示要显示的文字
	}
	system_task_return=0;//取消标志
	while(rval==0)
	{
		tp_dev.scan(0);    
		in_obj.get_key(&tp_dev,IN_TYPE_TOUCH);	//得到按键键值   
 		//在其他系统里面,用户可以自行去掉此句
 		if(system_task_return)//标志返回
		{	
