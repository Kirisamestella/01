...
//设置进度条的位置
//prgbx:进度条指针
void my_progressbar_setpos(_progressbar_obj *prgbx)
{
 	u16 prgblen;//进度柱的长度
	u16 x,y,xlen,ylen;
	u8 prgbtlen=0;//浮标长度
	...
	if(prgbx->curpos>prgbx->totallen)prgbx->curpos=prgbx->totallen;//范围限定	
	if((prgbx->type&0x03)==PRGB_TYPE_TEXT)prgbtlen=0;//浮标长度//百分数进度条
    else if((prgbx->type&0x03)==PRGB_TYPE_STD)prgbtlen=prgbx->prgbarlen;//浮标长度//标准的


	if(prgbx->type&PRGB_TYPE_VER)//垂直进度条
	{
        
        prgblen=((float)prgbx->curpos/prgbx->totallen)*(prgbx->height-2);//计算当前进度条的长度
        xlen=prgbx->width-2;
        x=prgbx->left+1;
        y=prgbx->top+1;
        if(prgbx->type&PRGB_TYPE_FEN)//浮标
        {
            if((prgbx->type&0x03)==PRGB_TYPE_STD);//标准进度条
            else ;
            ylen=prgbx->height-2-prgblen-prgbtlen/2;
            gui_fill_rectangle(x,y+ylen+1,xlen,prgbtlen-2,prgbx->btncolor);
			gui_draw_hline(x,y+ylen,xlen,prgbx->rimcolor);
			gui_draw_hline(x,y+prgbtlen-1,xlen,prgbx->rimcolor);
        }
        else ylen=prgbx->height-2-prgblen;
		gui_fill_rectangle(x,y,xlen,ylen,prgbx->inbkcolora);//背景

			
        if(prgbx->type&PRGB_TYPE_FEN)ylen=prgbx->height-2-prgblen+prgbtlen-prgbtlen/2;
        y=prgbx->top+1+ylen;
        ylen=prgbx->height-2-ylen;
        gui_fill_rectangle(x,y,xlen,ylen,prgbx->infcolora);//进度
...
void test_prgb(u16 x,u16 y,u16 width,u16 height,u8 type,u16 icur)
{
    ...
    prgb->inbkcolora=BLUE; 				//内部背景颜色
	prgb->inbkcolorb=BLUE; 				//内部背景颜色
	prgb->infcolora=GREEN; 					//内部填充颜色a
	prgb->infcolorb=GREEN; 					//内部填充颜色b
 	prgb->btncolor=RED;					//浮标填充色(标准prg时为内部滑动点的颜色,百分数prg时,为字体颜色,扩展时,无效)
	prgb->rimcolor=YELLOW;		
    prgb->prgbarlen=20;
	prgb->totallen=100;
	prgb->curpos=icur;
		 
	progressbar_draw_progressbar(prgb);//画	 
	progressbar_delete(prgb);//删	 
}



#define FLAGE_ON_OFF  0
#define VALUE_PRECENT 1
#define VER_HOR       2

#define BAR_HOR_PRE_OF       0 //水平方向、百分比、关浮标
#define BAR_HOR_PRE_ON       1 //水平方向、百分比、开浮标
#define BAR_HOR_VAL_OF       2 //水平方向、数值型、关浮标
#define BAR_HOR_VAL_ON       3 //水平方向、数值型、开浮标
#define BAR_VER_PRE_OF       4 //垂直方向、百分比、关浮标
#define BAR_VER_PRE_ON       5 //垂直方向、百分比、开浮标
#define BAR_VER_VAL_OF       6 //垂直方向、数值型、关浮标
#define BAR_VER_VAL_ON       7 //垂直方向、数值型、开浮标

u8 draw_progressbar(u16 sx,u16 sy,u16 ex,u16 ey,u16 bar_value_or_precent,u16 flage_val,u8 bar_type,u16 rectanglecolor, u16 barcolor,u16 backcolor,u16 flagecolor)
{
    u16 xlen,ylen;
    ...
    switch(bar_type)
    {
        case BAR_VER_VAL_ON://垂直方向、数值型、开浮标
        {
            LCD_DrawRectangle(sx, sy, ex, ey,rectanglecolor);		   				//画矩形
            LCD_Fill(sx+1,sy+1,ex-1,ey-flage_val-bar_value_or_precent-1,backcolor);
            LCD_DrawLine(sx+1, ey-flage_val-bar_value_or_precent,ex-1, ey-flage_val-bar_value_or_precent,rectanglecolor);
            LCD_Fill(sx+1,ey-flage_val-bar_value_or_precent+1,ex-1,ey-bar_value_or_precent-2,flagecolor);
            LCD_DrawLine(sx+1, ey-bar_value_or_precent-1,ex-1, ey-bar_value_or_precent-1,rectanglecolor);
            LCD_Fill(sx+1,ey-bar_value_or_precent,ex-1,ey-1,barcolor);    
        }
        break;
        default:break;
    }   
