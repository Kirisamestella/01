...
u8 USART1_TX_BUF[USART1_SED_LEN];     //接收缓冲,最大 个字节.
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
...
{  	 
	...
	GPIOA->CRH&=0XFFFFF00F;//IO状态设置,(我)定义GPIOA第九、十位为复用推挽输出和上下拉输入模式
...
{  
  ...
  for(t=0;t<len;t++)
  {
	...
	//while(DMA1_Channel7->CNDTR!=0);	//等待通道4传输完成   
...
{  	 
	...
	USART3->CR1|=0X200C;  	//1位停止,无校验位.PB10与LCD的D10冲突导致屏幕板变色
	
	USART3->CR3=1<<7;   	//使能串口3的DMA发送
	UART3_DMA_Config(DMA1_Channel2,(u32)&USART3->DR,(u32)USART3_TX_BUF);//DMA1通道12,外设为串口3,存储器为USART3_TX_BUF 
...
//	UART3_DMA_Enable(DMA1_Channel2,strlen((const char*)USART3_TX_BUF)); 	//通过dma发送出去
}
...
{  	 
	...
	MY_NVIC_Init(0,3,UART4_IRQn,2);//组2，最低优先级 
#endif
}
